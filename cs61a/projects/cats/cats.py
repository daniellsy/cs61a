...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def conclude(sent):
        a=split(remove_punctuation(lower(sent)))
        for i in topic:
            if i in a:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    if len(typed_words)==0 and len(reference_words)==0:
        ...
        return 0.0
    return sum(1 for i in range(len(typed_words)) if i<len(reference_words) and typed_words[i]==reference_words[i])/len(typed_words)*100
    # END PROBLEM 3
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    w_list=[[] for i in player_indices]
    for i in word_indices:
        times=[time(match,j,i) for j in player_indices]
        w_list[times.index(min(times))].append(word_at(match,i))
    return w_list
    # END PROBLEM 10
